import type { FnReturnType } from '@/store';
import { ILyric, parseLyric } from '@/utils/parse-lyric';
import { createSlice, createAsyncThunk } from '@reduxjs/toolkit';
import { getSongDetail, getSongLyric } from '../service/player-service';
...
export const fetchCurrentSongAction = createAsyncThunk<void, number, IThunkState>(
  ...
  (id: number, { dispatch, getState }) => {
    // 准备播放某一首歌曲时, 分成两种情况
    // 1.从列表尝试是否可以获取到这首歌
    const playSongList = getState().yyplayerReducer.playSongList;
...
export const changeMusicAction = createAsyncThunk<void, boolean, IThunkState>(
  ...
  (isNext, { dispatch, getState }) => {
    // 1.获取state中的数据
    const player = getState().yyplayerReducer;
